    ...
    if ($action === 'add') {
        ...
        if ($name && $email && $pass) {
            try {
                ...
                )->execute([$id, $name, $email, $pass, $role]);
...
<div style="background:<?= $t==='ok'?'var(--safe-bg)':'var(--crit-bg)' ?>;color:<?= $t==='ok'?'var(--safe)':'var(--crit)' ?>;border-radius:10px;padding:12px 16px;margin-bottom:20px;font-size:13px;font-weight:500;">
    <?= htmlspecialchars($m) ?>
</div>
<?php endif; ?>
...
<div class="card">
    <div class="table-wrap">
        <table>
            ...
            <tr>
                <td style="font-family:var(--font-head);font-size:12px;color:var(--gray-400);"><?= $u['id'] ?></td>
                <td>
                    ...
                    <div style="display:flex;gap:6px;">
                        ...
                        <form method="POST" style="display:inline;">
                            ...
                            <button type="submit" class="btn btn-danger btn-sm"
                                    onclick="return confirm('Delete user <?= addslashes($u['name']) ?>?')">
                                Delete
                            </button>
                        </form>
                    </div>
