...
const BASE_URL = document.documentElement.dataset.base || '/librarysaba';
...
function refreshZoneLevels() {
    fetch(`${BASE_URL}/api/zone_levels.php`, { cache: 'no-store' })
        ...
        .then(zones => {
            zones.forEach(z => {
                const fill = document.querySelector(`[data-zone="${z.id}"] .zone-prog-fill`);
                const num  = document.querySelector(`[data-zone="${z.id}"] .zone-db-num`);
                const card = document.querySelector(`[data-zone="${z.id}"]`);
                if (!fill || !num || !card) return;
                ...
                if (z.status === 'critical' && card.dataset.lastStatus !== 'critical') {
                    Toast.show('⚠️ Noise Alert', `${z.name} exceeded critical threshold (${z.level} dB)`, 'crit', 8000);
                }
